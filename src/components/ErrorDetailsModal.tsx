...
export const ErrorDetailsModal = ({ isOpen, onClose, jobName, errors }: ErrorDetailsModalProps) => {
  const getSeverityBadge = (severity: string) => {
    switch (severity) {
      case 'high':
        return <Badge className="bg-ike-error text-white">High</Badge>;
      case 'medium':
        return <Badge className="bg-ike-warning text-white">Medium</Badge>;
      case 'low':
        return <Badge className="bg-ike-success text-white">Low</Badge>;
      default:
      ...
      <DialogContent className="max-w-4xl max-h-[80vh] overflow-y-auto">
        <DialogHeader>
          <DialogTitle className="flex items-center text-ike-neutral-dark">
            ...
            Error Details - {jobName}
          </DialogTitle>
          <DialogDescription>
            Review and analyze calculation errors that occurred during job execution
          </DialogDescription>
        </DialogHeader>
              ...
              <div key={error.id} className="border rounded-lg p-4 bg-ike-error/5">
                <div className="flex items-start justify-between mb-2">
                  <div className="flex items-center space-x-2">
                    <h4 className="font-medium text-ike-neutral-dark">{error.municipality}</h4>
